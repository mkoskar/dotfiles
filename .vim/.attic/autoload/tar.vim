...
"  call Dfunc("tar#Browse(tarfile<".a:tarfile.">)")
  ...
  if !executable(g:tar_cmd)
   redraw!
   echohl ErrorMsg | echo '***error*** (tar#Browse) "'.g:tar_cmd.'" not available on your system'
   let &report= repkeep
...
"   call Decho('a:tarfile<'.a:tarfile.'> not filereadable')
   if a:tarfile !~# '^\a\+://'
    ...
    redraw!
    echohl ErrorMsg | echo "***error*** (tar#Browse) File not readable<".a:tarfile.">" | echohl None
   endif
   let &report= repkeep
...
"   call Dret("TarBrowseSelect")
  ...
  if fname =~ '/$'
"   redraw!
"   echohl ErrorMsg | echo "***error*** (tar#Browse) Please specify a file, not a directory" | echohl None
"   call inputsave()|call input("Press <cr> to continue")|call inputrestore()
   let &report= repkeep
"   call Dret("TarBrowseSelect")
   return
  ...
  exe "e ".fnameescape("tarfile:".tarfile.'::'.fname)
  filetype detect
...
"   call Dret("tar#Write")
  ...
  if !exists("*mkdir")
   redraw!
   echohl ErrorMsg | echo "***error*** (tar#Write) sorry, mkdir() doesn't work on your system" | echohl None
   let &report= repkeep
"   call Dret("tar#Write")
   return
  endif
...
"  call Decho("tmpdir<".tmpdir.">")
  ...
  try
   ...
   redraw!
   echohl ErrorMsg | echo "***error*** (tar#Write) cannot cd to temporary directory" | Echohl None
   let &report= repkeep
"   call Dret("tar#Write")
   return
  endtry
...
"  call Decho("tarfile<".tarfile.">")
  ...
  if v:shell_error != 0
   redraw!
   echohl ErrorMsg | echo "***error*** (tar#Write) sorry, unable to update ".tarfile." with ".fname | echohl None
  else
...
"   call Decho("system(".g:tar_cmd." --delete -f ".shellescape(tarfile,0)." -- ".shellescape(fname,0).")")
   ...
   if v:shell_error != 0
    redraw!
    echohl ErrorMsg | echo "***error*** (tar#Write) sorry, unable to update ".fnameescape(tarfile)." with ".fnameescape(fname) | echohl None
   else
